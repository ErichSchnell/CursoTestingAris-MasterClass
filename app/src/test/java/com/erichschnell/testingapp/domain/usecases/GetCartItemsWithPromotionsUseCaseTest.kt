...
class GetCartItemsWithPromotionsUseCaseTest {
    ...
    @Test
    fun `given cart with no items when invoke then it should return empty list`() = runTest {
        //Given
        val cartRepo = FakeCartRepository().apply { setCartItems(emptyList()) }
    ...
    @Test
    fun `given existing cart item with active promotion when invoke then it should return cart item with promotion`() = runTest {
        //Given
        val now = clock.now()
    ...
    fun `given cart item without matching product when invoke then skip item`() = runTest {
        ...
        val cartRepo = FakeCartRepository().apply { setCartItems(listOf(cartItem { withProductId("gosth-id") })) }
    ...
    @Test
    fun `given promotion ending exactly now when invoke then it must be included in the result`() = runTest {
        //Given
        val now = clock.now()
        val product = product { withId("product-id") }
        val promotion = promotion {
    ...
    @Test
    fun `given promotion ended when invoke then item remains but without promotion`() = runTest {
        //Given
        val now = clock.now()
        val product = product { withId("product-id") }
        val promotion = promotion {
