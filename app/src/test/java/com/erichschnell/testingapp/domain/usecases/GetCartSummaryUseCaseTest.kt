...
class GetCartSummaryUseCaseTest {
    ...
    fun `given percent promotion when invoke then calculate correctly`() = runTest {
        val product = product { withId("product-id"); withPrice(100.0) }
        ...
        val cart = cartItem { withProductId(product.id); withQuantity(2) }
    ...
    @Test
    fun `given 3 items in 2x1 promotion whe invoke then only discount 1 unit`() = runTest {
        val product = product { withId("product-id"); withPrice(100.0) }
        val promotion = promotion {
            withProductIds(listOf(product.id))
        ...
        val cart = cartItem { withProductId(product.id); withQuantity(3) }
    ...
    @Test
    fun `given multiple products with different promotions when invoke then calculate correctly`() = runTest {
        val p1 = product { withId("p1"); withPrice(100.0) } //con promo
        val p2 = product { withId("p2"); withPrice(50.0) } // sin promo

        val promotion = promotion {
            withProductIds(listOf(p1.id)); withType(PromotionType.PERCENT); withValue(10.0)
            withStartTime(clock.now().minusSeconds(10)); withEndTime(clock.now().plusSeconds(10))
        }
        val carts = listOf(
            cartItem { withProductId(p1.id); withQuantity(1) },
            cartItem { withProductId(p2.id); withQuantity(1) }
        )
    ...
    @Test
    fun `given expired promotion when invoke then discount not applied`() = runTest {
        val p1 = product { withId("p1"); withPrice(100.0) }

        val promotion = promotion {
            withProductIds(listOf(p1.id)); withType(PromotionType.PERCENT); withValue(10.0)
            withStartTime(clock.now().minusSeconds(10)); withEndTime(clock.now().minusSeconds(1))
        }
        val carts = listOf(cartItem { withProductId(p1.id); withQuantity(1) })
    ...
    fun `given active promotion when time advances then summary update automatically`() = runTest {
        ...
        val promotion = promotion {
            withProductIds(listOf(p1.id)); withType(PromotionType.PERCENT); withValue(10.0)
            withStartTime(clock.now().minusSeconds(5)); withEndTime(clock.now().plusSeconds(5))
        }
        val carts = listOf(cartItem { withProductId(p1.id); withQuantity(1) })
